...

{
	...
	bool Raycast(const Vector3 &startPosition, const Vector3 &direction,RayResult &output)
{
		if (!m_pWorld)
			return false;

		// get the picking ray from where we clicked
		btVector3 rayTo = btVector3(direction.x,direction.y,direction.z) *1000.0f;
		btVector3 rayFrom = btVector3(startPosition.x,startPosition.y,startPosition.z);
		...
		if (rayCallback.hasHit())
		{
			...
			output.pBody = pBody;
			//output.hitPoint = pBody->getWorldTransform().getOrigin() * rayCallback.m_hitPointWorld ;
...
void ShootBox(const Vector3 &direction,const Vector3 &cameraPosition)
{
		// create a new box object
		GameObject* pObject = CreateGameObject(new btBoxShape(btVector3(1, 1, 1)), 1, btVector3(0.4f, 0.f, 0.4f), btVector3(cameraPosition.x,cameraPosition.y,cameraPosition.z));

		// calculate the velocity
		btVector3 velocity = btVector3(direction.x,direction.y,direction.z);
		velocity.normalize();
		velocity *= 25.0f;
...
int main(void)
{
    ...
    while (!WindowShouldClose())                // Detect window close button or ESC key
    {
        // Update
                ...
                {
                  ShootBox(ray.direction,ray.position);
                }
                ...
                {
                RayResult result;
  				if (Raycast(ray.position,ray.direction, result))
             ...
             if (IsMouseButtonDown(MOUSE_BUTTON_RIGHT))
                {
                    RayResult result;
  			    	if (Raycast(ray.position,ray.direction, result))
					{
					DrawSphere(btor(result.hitPoint),0.5f,RED);
					//TraceLog(LOG_INFO,"%f %f %f",result.hitPoint.getX(),result.hitPoint.getY(),result.hitPoint.getZ());
					}
